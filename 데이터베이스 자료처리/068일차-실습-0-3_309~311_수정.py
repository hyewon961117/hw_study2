...
class GoodsRead :
    ...
    def goodsUpdate(self):
        try:
            ...
            update_code = int(input('수정할 코드 입력 : '))
            cursor.execute(f'select * from goods where code={update_code}')
            rows = cursor.fetchall()
            if rows:   
                print('<<<상품 코드 조회 결과입니다.>>>')  
                for row in rows:
                    print(row[0], row[1], row[2], row[3])
                answer = input("수정하시겠습니까?(Y/N) : ").lower()
                if answer == 'y':
                    print('<<<수정할 내용을 입력하세요.>>>')
                    update_name = input("상품명 : ")
                    update_su = int(input('수량 : '))
                    update_dan = int(input('단가 : '))
                    
                    update_sql = f"update goods set name='{update_name}', su={update_su}, dan={update_dan} where code={update_code}"
