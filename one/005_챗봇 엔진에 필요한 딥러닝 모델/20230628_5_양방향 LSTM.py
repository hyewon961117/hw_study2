...
# 모델 평가
X, y = get_sequence(n_timesteps)
# yhat = model.predict_classes(X, verbose=0) # 교재 에러
# yhat = model.predict(X, verbose=0).argmax(axis=-1) # 결과가 교재와 다르게 나옴 (다중분류)
yhat = (model.predict(X, verbose=0) > 0.5).astype('int32') # (이진분류)
