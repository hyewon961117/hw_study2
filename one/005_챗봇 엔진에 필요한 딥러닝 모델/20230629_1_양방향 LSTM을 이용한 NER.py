...
# 만들어진 단어 시퀀스와 태그 시퀀스를 사전으로 만들기 위해 토크나이저 정의
...
# 시퀀스 패딩 처리
max_len=40
X_train = tf.keras.utils.pad_sequences(X_train, padding='post', maxlen=max_len)
y_train = tf.keras.utils.pad_sequences(y_train, padding='post', maxlen=max_len)
...
new_padded_seqs = tf.keras.utils.pad_sequences([new_x],padding='post',value=0, maxlen=max_len)
